...
const manifestPath = resolve(root, 'local-lab-manifest.json');
if (Bun.argv.includes('--check')) {
  if (await Bun.file(manifestPath).text() !== serialized)
    throw new Error('Local Lab manifest is stale; regenerate with npm run manifest:local and review the changes');
  console.log(`Local Lab manifest is current: ${files.length} reviewed files`);
} else {
