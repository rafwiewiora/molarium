...
async function verifyHead(entry) {
  const response = await fetch(urlFor(entry), { method:'HEAD', headers:{ Origin:'https://molarium.org' } });
  if (!response.ok) throw new Error(`${entry.key}: HTTP ${response.status}`);
  if (Number(response.headers.get('content-length')) !== entry.bytes)
    throw new Error(`${entry.key}: Content-Length mismatch`);
...
for (let start = 0; start < manifest.files.length; start += 4)
  await Promise.all(manifest.files.slice(start, start + 4).map(verifyHead));
