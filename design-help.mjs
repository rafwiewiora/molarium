...
export const DESIGN_HELP = {
  ...
  'prepare-pdb': {title:'Prepare the molecular system', text:'Apply the selected protonation, ligand, water, gap and repair policies; add hydrogens and parameterize the system. This is not full-system minimization. Check the preparation report before capturing a design reference.'},
  'minimize-before-dynamics': {title:'Minimize before first simulation', text:'For the current system, minimize with the chosen force field and environment before dynamics. Reuse a matching minimized state or exact saved MD frame; molecular edits, other coordinate changes or different energy settings require a fresh minimum. Ligand-only and pocket relaxation do not qualify the full system. Current-system ensembles use WebGPU for this preflight. Unchecking explicitly skips it and can allow unstable starts; minimization is not equilibration or a guarantee of binding.'},
