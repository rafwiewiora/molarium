...
\begin{figure}[htbp]
  ...
  \caption{Calculation-free review of five exact, hash-pinned SOS1 design states in the full Molarium interface. Starting from a blank canvas, each state is loaded by the public \texttt{campaign.import} action. The first state establishes one pocket camera; the later imports preserve it. All panels show the network-locked Local Lab. (A) The exact prepared 5OVE/AXE starting hit, reconstructed from the registered root history prefix; 5OVE/AXE was the sole coordinate-bearing input. (B) The frozen scaffold-rewrite state (AWT). (C) The frozen fragment-merge state (AWZ). (D) The frozen compound-21/AWW prediction with the predicted Phe890-out open pocket. (E) The relaxed final BAY-293/AXH frozen prediction. Ligands are purple, neighboring pocket atoms are gray, and Phe890 is gold. The panels are saved full-system campaign checkpoints, not interpolated structures, and stepping through them performs no scientific recomputation. Post-freeze comparison with 5OVI was evaluated separately and did not seed any displayed state; this figure presents the complete frozen prediction sequence without labeling it as experimentally accepted.}
  \label{fig:sos1story}
\end{figure}
\clearpage

The purpose of this replay is to make a medicinal-chemistry argument inspectable rather than merely animate a structural series. Starting from the sole coordinate-bearing input, 5OVE/AXE, it exposes each public action that changes the ligand graph, selects a pose, tests the Phe890 conformational hypothesis, relaxes the complex, and freezes a prediction; post-freeze accuracy is evaluated separately. Exact saved checkpoints support calculation-free review, while the executable route reruns the underlying operations. The final state only partially recovers 5OVI: the Phe890 side chain and conserved ligand core have RMSDs of 0.682 and 0.537~\AA{}, respectively, whereas the complete ligand and distal feature have RMSDs of 2.897 and 4.841~\AA{}. This demonstrates how Molarium can visualize and reproduce the design logic of medicinal- and computational-chemistry papers, including results that only partly recover the experimental structures.
...
The selected replay is generated from the complete frozen run's pre-freeze API audit rather than from a hand-maintained action list. Exploratory branches and coordinate-bearing inspection results remain in the audit but are not embedded in the selected replay. Later crystal structures are opened only after all four prediction checkpoints and the exploration audit have been hashed. Display and focus actions are non-mutating.

Graph edits are transferred by chemistry rather than by spatial coincidence or atom-list position. Element- and bond-order-exact atoms in a conserved bond graph may remain hard coordinate anchors. An attachment rewire or ring-system replacement automatically releases the complete changed region for sampling while retaining the unchanged external scaffold. Compatible donor or acceptor roles may transfer as soft pharmacophore restraints across a bioisosteric edit, including across elements, but cannot establish atom identity. This rule is used for every registered design route; it is not specific to SOS1 or to the rendered replay.
...
Each pose-search chain is an independently seeded local search, not a protein chain. The frozen-run manifest records the search width and execution policy used at every stage. Parameterization without motion assigns the numerical force-field representation while preserving coordinates; only the subsequent relaxation changes coordinates. The SOS1 route uses whole-system Sage 2.1.0 with deterministic Gasteiger charges. The exact particle counts and charge assignment remain attached to the frozen run rather than being copied into a hand-maintained narrative.
...
3 & Grow compound 21 (AWW) & New growth enters the Phe890-in volume; evaluate the resulting ligand--pocket contacts\\
...
5 & Predict BAY-293 (AXH) & Grow from frozen AWW; stratified pose search; apply; parameterize without motion; relax; freeze\\
6 & Evaluate holdouts & Compare AWT/AWZ/AWW/BAY-293 with 5OVF/5OVG/5OVH/5OVI\\
\bottomrule
\end{tabularx}
\end{table}

The Phe890 conformational step is treated as a discrete state search because local minimization does not reliably cross the relevant side-chain torsional barrier. Complete $\chi_1/\chi_2$ candidates are generated before coupled ligand--pocket pose search and relaxation. Branches are ranked from post-relaxation coordinate evidence, with chemically invalid states rejected before score comparison. The selected basin and its alternatives are recorded before 5OVH or 5OVI is revealed.

For post-freeze structural evaluation, each predicted receptor is aligned to its holdout using the common pocket C$\alpha$ atoms. Ligand RMSD is then calculated over the complete registered heavy-atom graph, minimizing only over graph automorphisms; the ligand itself is not fitted. The evaluator also checks hard-core geometry, released and seed-only regions, Phe890 side-chain geometry, covalent integrity, severe clashes, and AWW-to-AXH continuity. Each reported result retains its per-state acceptance status; freezing a complete prediction does not imply that it passed the later holdout evaluation.
