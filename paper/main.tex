...
\input{generated/sos1-designer-intent-results.tex}
...
\begin{figure}[htbp]
  ...
  \caption{Designer intent and receptor response in the real Molarium interface. Six frames from the calculation-free, hash-pinned checkpoint movie show (A) the prepared 5OVE/AXE hit, (B) AWZ, (C) the AWW graph with its inherited arm direction, (D) the designer-directed AWW pose before receptor movement, (E) the energy-selected Phe890 response with the ligand fixed, and (F) the BAY-293 continuation. The full movie also includes the intervening scaffold-rewrite checkpoint. The pocket camera is preserved, the 2D graph remains visible, and the Local Lab is network-locked. Purple ligands, gold Phe890, and the labeled Tyr884 backbone contact distinguish the ligand and receptor decisions. The intent is crystal-informed but no later crystal coordinates are supplied to the replay.}
  \label{fig:sos1story}
\end{figure}
\clearpage

The purpose of this replay is to make a medicinal-chemistry argument inspectable rather than merely animate a structural series. Starting from the sole coordinate-bearing input, 5OVE/AXE, it exposes the public actions that change the ligand graph, preserve the precursor outside a declared moving branch, record the AWW direction and Tyr884 contact, lock the ligand, and calculate the Phe890 response. The final graph rewrite then retains the distal spatial feature while rebuilding the BAY-293 attachment. Exact checkpoints support calculation-free review; a separate executable script records the same scientific operations through the public API. The AWW and BAY-293 ligand RMSDs are \SosIntentLigandRmsd{} and \SosIntentAxhLigandRmsd{}~\AA{}, respectively, in a receptor-aligned, graph-symmetry-minimized comparison without ligand fitting. These reference-informed examples demonstrate inspectable design logic, not blinded predictive accuracy.

The accompanying source package contains the executable script, the calculation-free checkpoint-review script, their exact coordinate campaigns, and the full-interface movie. Their content hashes identify this result independently of changes to the live Molarium website.
...
Executable replays use \texttt{molarium.chemist-action-script/v1}. Scripts contain ordered calls to the allow-listed Chemist Actions API rather than arbitrary code or direct coordinate replacement. Designer Moves JSON files must be smaller than 2 MB. Ordinary API requests are capped at 8 MiB for ASCII JSON, twelve nested levels, and 2,048 JSON nodes. Loading a multi-action script permits 32,768 nodes while retaining the depth and byte limits; each constituent action still uses the ordinary request limits. Serialized full-system campaign imports have a separate 32 MiB byte allowance. Actions can name returned stable identifiers for reuse by later steps. The runner executes one action at a time through \texttt{api.execute(\{action,args\})}, derives a repeatable request ID from the script fingerprint, and stops on the first failure. Replay results use \texttt{molarium.chemist-action-replay/v1} and record the source fingerprint and each step outcome. Presentation actions can alter focus or styling but cannot change molecular graph or coordinates except through the same allow-listed state-changing actions.
...
\subsection{SOS1 designer-intent replay}

The SOS1 example follows the analogue series reported by Hillig and colleagues \citep{hillig2019sos1}. Its coordinate-generating workflow starts from the 5OVE/AXE complex and the published analogue graphs for AWT, AWZ, AWW, and AXH. The later crystal series was inspected to infer the designer's intended AWW exit direction and interaction hypothesis. This demonstration is therefore reference-informed, not a blinded reproduction. Later crystal coordinates are not used as coordinate targets or imported into the design trajectory.

The executable replay is generated from the frozen runs' API audits rather than from a hand-maintained action list. It retains all 13 Phe890 candidate applications, energy calculations, and restoration actions. Inspection requests are replayed, but their saved coordinate results are not embedded as inputs. Portable ligand-axis selectors resolve named atoms in the current component; canonical side-chain candidates use chi angles, whereas the unchanged control selects the current input conformation. Recomputed coordinates and energies may differ from the frozen record, so movie export requires the recorded final response to remain the lowest-energy clash-free candidate under the fresh calculations. The separate checkpoint review imports seven exact saved states without recalculation. The designer-directed AWW state is frozen before receptor selection, and the selected receptor state is hashed before numerical comparison with 5OVH. This ordering prevents post-comparison coordinate edits within the recorded run; it does not erase the prior inspection used to formulate the hypothesis. Display and focus actions are non-mutating.

Graph edits are transferred by chemistry rather than by spatial coincidence or atom-list position. Element- and bond-order-exact atoms in a conserved bond graph retain explicit lineage. In the AWW workflow, the precursor coordinates outside the declared moving branch are preserved. The chemist uses the public \texttt{geometry.alignBranchToContact} action to specify forward rotation about N7--C12 within $0$--$60^{\circ}$, a $+150^{\circ}$ primary rotation about C12--C15, and the intended OX3 donor contact to the Tyr884 backbone carbonyl oxygen. Two downstream torsions and the donor hydrogen orientation complete the bounded contact search. Atom selectors and every generated coordinate derive from the current structure; no later crystal coordinate is a target. Candidates must clear fixed receptor atoms and retained source waters. Only the six movable Phe890 ring atoms may initially overlap the ligand; the immovable C$\beta$ atom is not exempt.

The resulting AWW ligand pose is an explicit designer hypothesis inferred from the series' reported interaction logic, not an attempt to reproduce deposited ligand coordinates exactly. Once that decision is recorded, the ligand coordinates are locked. The response calculation varies only the Phe890 side chain while the ligand, backbone, other receptor atoms, and source waters remain fixed. This separation prevents a receptor-response calculation from silently rewriting the chemist's ligand hypothesis. Tyr884 supplies a backbone-carbonyl contact; a Tyr884 side-chain flip is neither required nor modeled.
...
\begin{table}[htbp]
\centering\small
\caption{Reference-informed construction and evaluation boundary for the AWW designer-intent replay.}
...
2 & Record interaction intent & Declare the AWW OX3 donor--Tyr884 backbone-carbonyl-oxygen contact\\
3 & Direct AWW geometry & Bound the forward N7--C12 rotation, apply the $+150^{\circ}$ primary rotation, and solve coupled contact torsions using current coordinates only\\
...
6 & Freeze, then evaluate & Hash the result before numerical comparison with the previously inspected 5OVH structure\\
\bottomrule
\end{tabularx}
\end{table}

The Phe890 conformational step is treated as a discrete receptor-state search because local minimization does not reliably cross the relevant side-chain torsional barrier. All \SosIntentCandidateCount{} enumerated $\chi_1/\chi_2$ candidates received fixed-coordinate, full-system OpenMM single-point energy evaluations with OBC2 implicit solvent. Selection used the minimum finite energy among candidates with no severe clashes, yielding $(-180^{\circ},90^{\circ})$. Ligand coordinates remained exactly equal before and after every candidate calculation. The selected basin and all alternatives were recorded before numerical crystal comparison.

The selected Phe890 state clears the designer-directed ligand: OX3--Tyr884 O is \SosIntentContactDistance{}~\AA{}, the donor--hydrogen--acceptor angle is \SosIntentContactAngle{}$^{\circ}$, and no severe ligand-internal or ligand--protein clashes remain. The primary arm torsion differs from 5OVH by \SosIntentPrimaryDifference{}$^{\circ}$. Phe890 $\chi_1$ and symmetry-aware $\chi_2$ differ by \SosIntentChiOneDifference{}$^{\circ}$ and \SosIntentChiTwoDifference{}$^{\circ}$, respectively. These measurements describe the geometry produced by the recorded designer hypothesis and receptor response.

The BAY-293 continuation resumes this exact full-system AWW checkpoint through the public API, releases the ligand lock for the new graph edit, and applies the registered distal-feature retention rule. Eight native refinement candidates precede coupled relaxation with explicit fixed-atom and ligand-valence checks. The resulting 32-heavy-atom ligand has no severe internal or protein clashes. Its Phe890 side-chain RMSD to 5OVI is \SosIntentAxhPheRmsd{}~\AA{}. The seven-atom distal feature moves by \SosIntentAxhDistalRmsd{}~\AA{} RMSD and \SosIntentAxhDistalCentroid{}~\AA{} in centroid position relative to the AWW hypothesis, within the registered 2.25~\AA{} feature tolerance. This is bounded spatial retention, not an assertion that the distal ring is immobile or that the experimental H-to-I displacement was exactly recovered.
