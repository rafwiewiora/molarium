...
\begin{figure}[htbp]
  ...
  \caption{Hit-only SOS1 design replayed in the Molarium interface through the public Chemist Actions API. (A) The prepared 5OVE/AXE hit (teal) and its local pocket establish the camera used throughout; 5OVE/AXE was the sole coordinate-bearing input. (B) Sequential scaffold rewriting and growth produces compound 21/AWW (purple) in the inherited Phe890-in pocket. The cyan halo identifies the edited ligand region, gold identifies Phe890, and magenta dotted connectors mark severe ligand--protein short contacts recalculated from the live coordinates. (C) A discrete side-chain search presents alternatives and the designer chooses the Phe890-out hypothesis; only the six moved Phe890 heavy atoms are haloed. (D) In the opened pocket, a 64-chain restrained search re-fits compound 21, applies the selected feasible pose, and couples it to the receptor during WebGPU relaxation. The ligand displacement from B to D is a modeled coordinate change, not camera motion or an MD trajectory. Between D and E, AWW is changed to BAY-293/AXH by rebuilding its distal arm from the other thiophene attachment position. Exact element and conserved-bond correspondence holds the 15-heavy-atom proximal quinazoline--thiophene core fixed during staging; the 16-heavy-atom old distal region is deleted and the 17-heavy-atom AXH distal region is released for sampling. No element-agnostic atom correspondence is allowed. A new 64-chain pose search selects a feasible conformation and repositions nine released ligand heavy atoms (maximum displacement 5.90~\AA{}); coupled WebGPU relaxation then moves 49 ligand and nearby pocket heavy atoms above the 0.08~\AA{} reporting threshold (maximum displacement 1.19~\AA{}). (E) The resulting BAY-293 prediction is shown with the same camera. The replay ends at this prediction and never opens 5OVI. Independent post-freeze comparison with 5OVI subsequently gave a receptor-aligned, graph-symmetry-minimized ligand RMSD of 1.215~\AA{} and Phe890 $\chi_1$ values of $-172.43^{\circ}$ in the prediction and $-166.55^{\circ}$ in the crystal; those validation coordinates did not seed the replay.}
  \label{fig:sos1story}
\end{figure}
...
The selected route contains 33 recorded API actions: 29 molecular or protocol operations and four workspace switches. The full exploration audit contains 89 completed API calls, including three Phe890/pose branches, nine undo calls, and deterministic reselection. The rendered interface sequence adds 18 display-only actions for 51 replay steps. Later crystal structures were opened only after four prediction checkpoints and the exploration audit had been hashed. Display and focus actions are non-mutating.
