...
\begin{figure}[htbp]
  ...
  \caption{Hit-only SOS1 design replayed in the Molarium interface through the public Chemist Actions API. (A) The prepared 5OVE/AXE hit (teal) and its local pocket establish the camera used throughout; 5OVE/AXE was the sole coordinate-bearing input. (B) Sequential scaffold rewriting and growth produces compound 21/AWW (purple) in the inherited Phe890-in pocket. The cyan halo identifies the edited ligand region, gold identifies Phe890, and magenta dotted connectors identify severe ligand--protein short contacts recalculated from the live coordinates. (C) A discrete side-chain search presents alternatives and selects a Phe890-out hypothesis; only the moved Phe890 heavy atoms are haloed. (D) In the opened pocket, a stratified reference-guided pose search applies the selected feasible compound-21 pose and couples it to the receptor during WebGPU relaxation. The ligand displacement from B to D is a modeled coordinate change, not camera motion or a molecular-dynamics trajectory. Between D and E, the AWW graph is changed to BAY-293/AXH. Exact element and conserved-bond correspondence keeps an 11-heavy-atom core fixed; four mapped connector atoms are released because the attachment changes; and the seven-atom distal aromatic feature provides a starting placement but is not a coordinate restraint. Pose search and coupled relaxation may move only the released regions and nearby pocket. (E) The resulting BAY-293 prediction is shown with the same camera. The replay ends at the frozen prediction and never opens 5OVI. Figure generation is permitted only for a complete run that passes the independent post-freeze holdout and AWW-to-AXH continuity criteria.}
  \label{fig:sos1story}
\end{figure}
